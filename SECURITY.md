...
- **`api/server.js`** — forging or replaying a session cookie, bypassing password or passkey verification,
...
- **Nothing in `./data` is encrypted.** It holds `db.json` (users, scrypt password hashes, passkey public keys, push
...
- **User verification is preferred, not required.** Passkey handshakes pass
  `requireUserVerification: false`, so a passkey released without a biometric or PIN is still
  accepted. In practice: unlocked device ≈ account access for passkey login.
- **Passwords are scrypt-hashed** (`scrypt$N$r$p$salt$hash` in `db.json`). They are never sent
  back to the client. There is no self-serve reset: a gym owner or platform admin can set a new
  password, which bumps the session version and signs that person out everywhere.
- **One passkey per profile.** New profiles are created with email + password
  (`POST /api/register`). A passkey can be attached later while signed in
  (`POST /api/passkey/register/*`); attaching another replaces the previous one. Existing
  passkey-only accounts keep working. Lose both the password and the passkey and the gym owner
  can set a new password — or you edit `./data`.
- **Disabling someone isn't a ban.** They can still register a fresh profile with a new email
...
- **HTTPS is required and the app doesn't provide it.** The API container speaks plain HTTP and
  ...
  in the clear.
- **No rate limiting anywhere.** Nothing throttles logins, registrations or writes.
  `POST /api/register` tells a caller whether an email is already used (409) and whether a gym
  join code is valid (403), so an instance on the open internet should have a rate limit in
  front of it. Gym join codes default to 8 hex characters; owners may replace them with a
  memorable 4–24 character code (`A–Z`, `0–9`, hyphen), which is easier to guess — rotate it
  if it leaks. One-time admin invite codes are still 16 hex characters / 64 bits. The only
...
- **A few endpoints answer without a session:** `/api/health` (which includes the total user
  ...
  register/login handshakes (`POST /api/register`, `POST /api/login`, passkey login).
