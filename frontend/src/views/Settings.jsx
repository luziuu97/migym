...
import { api, webauthnOK, passkeyLogin, passkeyAttach, IS_ANDROID, BIO } from '../lib/api.js'
...
export default function Settings() {
  ...
  const { update, replaceState, setUser, pullState, signOut, signOutAll, resetDemo } = useStore()
  ...
  const registerHere = () => useUI.getState().openSheet(close => <RegisterSheet close={close} />)
  const addPasskeyHere = async () => {
    try {
      const u = await passkeyAttach()
      setUser(u)
  ...
  const signOutEverywhere = () => confirmSheet({
    ...
    message: t('Signs this profile out on every device, including this one. Sign in again with email and password, or a passkey.'),
  ...
  return <div className="narrow">
    ...
    <Section title={MOBILE ? t('Your data') : DEMO ? t('Demo') : t('Account')}>
      {MOBILE ? <>
        ...
        {user.hasPassword
          ? <Row icon="lock" iconTint="var(--blue)" title={t('Change password')} accessory="chevron" onClick={() => useUI.getState().openSheet(close => <PasswordSheet close={close} mode="change" />)} />
          : <Row icon="lock" iconTint="var(--blue)" title={t('Add email and password')} subtitle={t('Set a password so you can sign in on any device.')} accessory="chevron" onClick={() => useUI.getState().openSheet(close => <PasswordSheet close={close} mode="add" />)} />}
        {webauthnOK() && <Row icon="person" iconTint="var(--teal)" title={user.hasPasskey ? t('Replace passkey') : t('Add a passkey')}
          subtitle={user.hasPasskey ? t('Uses {0} on this device.', BIO) : t('Sign in with {0} instead of a password.', BIO)}
          accessory="chevron" onClick={addPasskeyHere} />}
        ...
        <Row icon="sparkles" iconTint="var(--acc)" title={t('Create new profile')} subtitle={t('Email and password — you can add a passkey later.')} accessory="chevron" onClick={registerHere} />
        {webauthnOK() && <Row icon="person" iconTint="var(--blue)" title={t('Sign in with passkey')} accessory="chevron" onClick={signInHere} />}
      </>}
    </Section>
...
function PasswordSheet({ close, mode }) {
  ...
  const add = mode === 'add'
  const go = async () => {
    if (add && !email.trim()) { toast(t('Enter an email')); return }
    if (!add && !current) { toast(t('Enter a password')); return }
    if (password.length < 8) { toast(t('Password must be at least 8 characters')); return }
    if (password !== confirm) { toast(t('Passwords do not match')); return }
    setBusy(true)
    try {
      const body = add
        ? { email: email.trim(), password }
        : { currentPassword: current, newPassword: password }
      const { user: u } = await api('/api/account/password', { method: 'POST', body: JSON.stringify(body) })
      setUser(u)
  ...
  return <>
    <h3>{add ? t('Add email and password') : t('Change password')}</h3>
    {add && <>
      <div className="muted small" style={{ marginBottom: 14 }}>{t('Set a password so you can sign in on any device.')}</div>
    ...
    {!add && <>
      <input className="input" type="password" placeholder={t('Current password')} value={current} onChange={e => setCurrent(e.target.value)} autoComplete="current-password" />
      <div style={{ height: 10 }} />
    </>}
    <input className="input" type="password" placeholder={t('New password')} value={password} onChange={e => setPassword(e.target.value)} autoComplete="new-password" />
    <div style={{ height: 10 }} />
    <input className="input" type="password" placeholder={t('Confirm password')} value={confirm} onChange={e => setConfirm(e.target.value)} autoComplete="new-password" />
    <div style={{ height: 12 }} />
    <Button variant="primary" onClick={go} disabled={busy}>{t('Save')}</Button>
  </>
}
